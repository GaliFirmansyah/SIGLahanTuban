<x-app-layout>
                    ...
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="border-2 border-gray-400 border-dashed hover:border-transparent hover:bg-white hover:shadow-xl rounded p-6 m-1 md:my-6">
                            <div class="flex flex-col items-center">
                                ...
                                <div class="flex-1">
                                    <h3 class="font-bold text-3xl text-center">{{ $lahan->count() }} <span class="text-green-500"></h3>
                                    <h5 class="font-bold text-gray-500">Total Lahan</h5>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <x-slot name="script">
        <script>
            ...
            var map = L.map('mapid').setView([-6.895180, 112.065183], 12);
            L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1IjoibHVheXN5YXVxaWxsYWgiLCJhIjoiY2sxeGt1ZHZlMDhyOTNsb2lxZWlxbmFsdiJ9.DO8qDjsP0y18UTAS5MxxXQ', {
                ...
                var greenIcon = L.icon({
                    iconUrl: {!! json_encode(asset('images/padi.png')) !!},
        ...
        $('.check').click(function () {
            ...
            for (var i = 0; i < a.length; i++) {
                ...
                .bindPopup("<h5 class='text-center text-green-500 mt-2 text-xs'>"+a[i].pemilik+"</h5><hr><p>"+a[i].alamat+".</p>")
                    .addTo(map);
