<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <ul class="flex text-gray-500 text-sm lg:text-base">
                ...
                <li class="inline-flex items-center">
                    <x-nav-link :href="route('lahan.edit',['lahan' => $lahan->id])" :active="request()->routeIs('lahan.edit',['lahan' => $lahan->id])">
                        {{ __('Lahans Edit') }}
                    </x-nav-link>
                    <svg class="h-5 w-auto text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class=" inline-block m-2 md:m-5 border border-blue-300 bg-blue-400 text-white md:py-1 md:px-5 p-1 rounded">
                        <a href="{{ route('lahan.index') }}"><i class="fas fa-arrow-alt-circle-left"></i> Back</a>
                    </div>
                    <h1 class="text-center text-4xl font-bold">Update Lahan</h1>
                    ...
                    <form action="{{ route('lahan.update',['lahan'=>$lahan->id]) }}" method="post">
                        ...
                        <label class="block my-3">
                            ...
                            <input type="text" class="mt-0 block w-full px-0.5 mx-5 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-blue-400" value="{{ $lahan->alamat }}" name="alamat" />
                        </label>
                        <label class="block my-3">
                            ...
                            <select class="mt-0 block w-full px-0.5 mx-5 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-blue-400" name="kecamatan">
                                @foreach ($kecamatan as $item)
                                    <option {{ $lahan->kecamatan==$item->id?'selected':'' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </label>
                        <label class="block my-3">
                            ...
                            <select class="mt-0 block w-full px-0.5 mx-5 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-blue-400" name="pemilik">
                                @foreach ($pemilik as $item)
                                    <option {{ $lahan->pemilik==$item->id?'selected':'' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </label>
                        <label class="block my-3">
                            <span class="text-gray-700">Latitude</span>
                            <input type="text" class="mt-0 block w-full px-0.5 mx-5 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-blue-400" id="lt" value="{{ $lahan->latitude }}" name="latitude" />
                        </label>
                        <label class="block my-3">
                            <span class="text-gray-700">Longitude</span>
                            <input type="text" class="mt-0 block w-full px-0.5 mx-5 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-blue-400" id="lg" value="{{ $lahan->longitude }}" name="longitude" />
                        </label>
    ...
    <x-slot name="script">
        <script>
            ...
            var latitude = {!! json_encode($lahan->latitude) !!}
            var longitude = {!! json_encode($lahan->longitude) !!}
